...
import { ArrowLeft, Building2, CalendarDays, Mail, MapPin, MoreHorizontal, Phone, Tag } from "lucide-react";
...
export default async function PersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <MobileShell><main className="mobile-page"><header className="topbar"><Link href="/network" className="icon-button"><ArrowLeft size={19} /></Link><span className="eyebrow">Connection</span><button className="icon-button"><MoreHorizontal size={20} /></button></header>
    ...
    <NoteEditor initial={person.note} /><section className="card card-pad section"><div className="section-head"><h2 className="section-title">태그</h2><button className="chip" style={{ border: 0 }}>+ 추가</button></div><div className="filter-row"><span className="chip violet"><Tag size={12} />{person.tags[0]}</span>{person.tags.slice(1).map((tag) => <span className="chip green" key={tag}>{tag}</span>)}</div></section>
  </main></MobileShell>;
}
